...
const FOLDER_PATH = "F:\\movie\\Telegram Desktop";
const PASSWORD = "qazwsx";

/**
 * Hidden access button, requires 5 clicks, then password to reveal folder tool
 */
export const HiddenAccess: React.FC<{ onReveal: () => void; show: boolean }> = ({ onReveal, show }) => {
  ...
  if (!show && !unlocked) return (
    <button
      ...
      onClick={() => setCnt(v => (v + 1 >= REVEAL_CLICK_COUNT ? (setUnlocked(true), 0) : v + 1))}
    >*</button>
  );
 ...
 */
export const HiddenFolderAccess: React.FC<{ onClose: () => void }> = ({ onClose }) => (
  <div className="fixed bottom-12 right-8 z-[999] bg-white/95 border rounded-xl p-5 shadow-lg flex flex-col gap-2">
    <div className="font-semibold">Special Folder Access:</div>
    <input disabled value={FOLDER_PATH} className="rounded px-2 py-1 font-mono w-full mb-2" />
    <button className="bg-accent px-3 py-1 rounded mb-2" onClick={() => window.open(`file://${FOLDER_PATH}`)}>
      Open Folder
    </button>
    <button className="text-xs underline text-accent-foreground" onClick={onClose}>
      Hide
    </button>
  </div>
)
